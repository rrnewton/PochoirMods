...
main = do args <- getArgs
          ...
          -- pass everything to icc after preprocessing and Pochoir
          -- optimization
...
ppopp (mode, debug, showFile, userArgs) ((inFile, inDir):files) = 
    do putStrLn ("pochoir called with mode =" ++ show mode)
       pochoirLibPath <- catch (getEnv "POCHOIR_LIB_PATH")
                         (\e -> return "EnvError")
...
getMidFile a  
    | isSuffixOf ".cpp" a || 
      isSuffixOf ".cxx" a = take (length a - 4) a ++ ".i"
    | otherwise = a
...
pInitState = 
  ParserState { pMode = PCaching, pState = Unrelated, 
                pMacro = Map.empty, pArray = Map.empty, 
                pStencil = Map.empty, pShape = Map.empty, 
                pRange = Map.empty, pKernel = Map.empty}

icc = "icpc"

iccFlags = 
  ["-O3", "-DNDEBUG", "-std=c++0x", "-Wall", "-Werror", "-ipo"]

iccPPFlags = 
  ["-P", "-C", "-DNCHECK_SHAPE", "-DNDEBUG", "-std=c++0x", 
   "-Wall", "-Werror", "-ipo"]
...
iccDebugPPFlags = 
  ["-P", "-C", "-DCHECK_SHAPE", "-DDEBUG", "-g3", "-std=c++0x"]    
  
data CMode = Default | Debug | Cacing | CPointer | OptPointer | Pointer
           | MacroShadow | NoPP

data Flag = Help | CompileMode CMode | ShowFile | InputFile String 
          | OutputFile String
         
options :: [OptDescr Flag]
options =
  [ Option ['h'] ["help"] (NoArg Help) "Usage for Pochoir.",
    
    Option [] ["auto-optimize"] (NoArg CompileMode Default) 
    "Let the Pochoir compiler automatically choose the best optimizing 
level for you! (default)",
    
    Option [] ["split-caching"] (NoArg CompileMode Caching)
    "This could use a useful description",
    
    Option [] ["split-c-pointer"] (NoArg CompileMode CPointer)
    "This could use a useful description",
    
    Option [] ["split-opt-pointer"] (NoArg CompileMode OptPointer)
    "This could use a useful description",
    
    Option [] ["split-pointer"] (NoArg CompileMode Pointer)
    "Default Mode : split the interior and boundary region, and using 
C-style pointer to optimize the base case",
    
    Option [] ["split-macro-shadow"] (NoArg CompileMode MacroShadow)
    "This could use a useful description",
    
    Option [] ["show-file"] (NoArg ShowFile)
    "This could use a useful description",
...
parseArgs :: ([String], [String], PMode, Bool, Bool, [String]) ->  
             [String] -> 
             ([String], [String], PMode, Bool, Bool, [String])
parseArgs (inFiles, inDirs, mode, debug, showFile, userArgs) aL 
    ...
    | elem "-split-caching" aL =
        ...
        in  parseArgs (inFiles, inDirs, l_mode, debug, showFile, aL') 
            aL'
    ...
    | elem "-showFile" aL =
        ...
        in  parseArgs (inFiles, inDirs, mode, debug, l_showFile, aL') 
            aL'
    | elem "-debug" aL =
        let l_debug = True 
            l_mode = PDebug
            aL' = delete "-debug" aL
        in  parseArgs (inFiles, inDirs, l_mode, l_debug, showFile, aL')
            aL'
    | null aL == False =
        let (l_files, l_dirs, l_mode, aL') = 
              findCPP aL ([], [], mode, aL)
...
findCPP :: [String] -> ([String], [String], PMode, [String]) -> 
           ([String], [String], PMode, [String])
findCPP [] (l_files, l_dirs, l_mode, l_al) = 
    let l_mode' = 
            if null l_files == True || null l_dirs == True then PNoPP 
            else l_mode
...
printOptions = 
    do putStrLn ("Usage: pochoir [OPTION] [filename]")
       ...
       putStrLn ("-auto-optimize : " ++ breakline ++ "Let the Pochoir 
compiler automatically choose the best optimizing level for you! 
(default)")
       putStrLn ("-split-macro-shadow $filename : " ++ breakline ++ 
                 "using macro tricks to split the interior and boundary 
regions")
       putStrLn ("-split-pointer $filename : " ++ breakline ++ 
                 "Default Mode : split the interior and boundary region,
and using C-style pointer to optimize the base case")
